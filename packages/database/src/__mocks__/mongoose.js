...
function extractSchemaInfo(schemaDefinition) {
  ...
  if (!schemaDefinition) return { requiredFields, defaults, enums, nestedSchemas };
  ...
  for (const [fieldName, fieldDef] of Object.entries(schemaDefinition)) {
    if (fieldDef && typeof fieldDef === 'object') {
      ...
      if (fieldDef.default !== undefined) {
        defaults[fieldName] = typeof fieldDef.default === 'function' 
          ? fieldDef.default() 
          : fieldDef.default;
      }
      ...
      if (fieldDef.type && Array.isArray(fieldDef.type) && fieldDef.type.length > 0) {
      ...
      if (fieldDef.type && typeof fieldDef.type === 'object' && !Array.isArray(fieldDef.type)) {
...
function validateNestedArray(fieldName, items, nestedSchemaInfo) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (const [field, allowedValues] of Object.entries(nestedSchemaInfo.enums)) {
      if (item[field] !== undefined && !allowedValues.includes(item[field])) {
        throw new Error(`${fieldName}[${i}].${field} must be one of: ${allowedValues.join(', ')}`);
      }
    }
  }
}
    ...
    constructor(data = {}) {
      ...
      for (const [field, defaultValue] of Object.entries(schemaInfo.defaults)) {
        if (data[field] === undefined) {
          this[field] = typeof defaultValue === 'object' && defaultValue !== null
      ...
      for (const [field, nestedSchemaInfo] of Object.entries(schemaInfo.nestedSchemas)) {
