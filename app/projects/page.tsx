...
interface Project {
  ...
  owner: string;
  collaborators: string[];
  createdAt?: string; // ✅ Added for created date
}
...
export default function ProjectsPage() {
  ...
  const popupRef = useRef<HTMLDivElement>(null);
  const [search, setSearch] = useState("");
  const [invitationCount, setInvitationCount] = useState(0); // ✅ Invitation count
  ...
  useEffect(() => {
    ...
    fetch(`/api/invitations?userEmail=${encodeURIComponent(user.email)}`)
      .then((res) => res.json())
      .then((data) => {
        if (Array.isArray(data)) setInvitationCount(data.length);
      })
